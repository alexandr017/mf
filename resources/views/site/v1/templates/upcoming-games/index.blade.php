@extends('site.v1.layouts.default')

@section('content')
<!-- Page Header -->
<section class="bg-gray-900 py-16 relative overflow-hidden">
    <div class="absolute inset-0">
        <img src="/v1/images/tournaments-banner.jpg" alt="Upcoming Games Background" class="w-full h-full object-cover opacity-30">
    </div>
    <div class="relative max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 text-center">
        <h1 class="heading-font text-5xl md:text-7xl text-white mb-6">Ближайшие матчи</h1>
        <p class="text-xl text-gray-200 mb-8 max-w-3xl mx-auto">Все матчи лиги: товарищеские игры, чемпионаты и турниры.</p>
    </div>
</section>

<!-- Filters -->
<section class="py-8 bg-gray-50">
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <form method="GET" action="/upcoming-games" id="filterForm" class="flex flex-col md:flex-row gap-4">
            <div class="flex-1">
                <label for="match_type" class="block text-sm font-medium text-gray-700 mb-2">Тип матча</label>
                <select name="match_type" id="match_type" class="w-full rounded-lg border-gray-300">
                    <option value="">Все матчи</option>
                    <option value="friendly" {{ request('match_type') === 'friendly' ? 'selected' : '' }}>Товарищеские</option>
                    <option value="tournament" {{ request('match_type') === 'tournament' ? 'selected' : '' }}>Официальные</option>
                </select>
            </div>
            <div class="flex-1">
                <label for="status" class="block text-sm font-medium text-gray-700 mb-2">Статус</label>
                <select name="status" id="status" class="w-full rounded-lg border-gray-300">
                    <option value="">Все матчи</option>
                    <option value="scheduled" {{ request('status') === 'scheduled' ? 'selected' : '' }}>Запланированные</option>
                    <option value="played" {{ request('status') === 'played' ? 'selected' : '' }}>Сыгранные</option>
                    <option value="cancelled" {{ request('status') === 'cancelled' ? 'selected' : '' }}>Отмененные</option>
                </select>
            </div>
            <div class="flex-1">
                <label for="date" class="block text-sm font-medium text-gray-700 mb-2">Дата</label>
                <input type="date" name="date" id="date" value="{{ request('date') }}" class="w-full rounded-lg border-gray-300">
            </div>
            <div class="flex items-end">
                <button type="submit" class="bg-primary text-gray-900 font-bold px-6 py-2 rounded-lg hover:bg-opacity-80 transition-colors">
                    Применить
                </button>
                @if(request()->has('status') || request()->has('date') || request()->has('match_type'))
                    <a href="/upcoming-games" class="ml-2 bg-gray-200 text-gray-700 font-bold px-6 py-2 rounded-lg hover:bg-gray-300 transition-colors">
                        Сбросить
                    </a>
                @endif
            </div>
        </form>
    </div>
</section>

<!-- Tabs -->
<section class="py-8 bg-white" id="tabsSection" style="{{ isset($hasFilters) && $hasFilters ? 'display: none;' : '' }}">
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div class="border-b border-gray-200">
            <nav class="-mb-px flex space-x-8" aria-label="Tabs">
                <button onclick="showTab('old')" id="tab-old" class="tab-button active border-b-2 border-primary text-primary py-4 px-1 text-sm font-medium">
                    Прошедшие
                </button>
                <button onclick="showTab('recent')" id="tab-recent" class="tab-button border-b-2 border-transparent text-gray-500 hover:text-gray-700 hover:border-gray-300 py-4 px-1 text-sm font-medium">
                    Вчера, сегодня, завтра
                </button>
                <button onclick="showTab('future')" id="tab-future" class="tab-button border-b-2 border-transparent text-gray-500 hover:text-gray-700 hover:border-gray-300 py-4 px-1 text-sm font-medium">
                    Будущие
                </button>
            </nav>
        </div>
    </div>
</section>

<!-- Matches List -->
<section class="py-16 bg-white">
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <!-- Old Matches Tab -->
        <div id="tab-content-old" class="tab-content">
            @if($oldMatchesByDate->count() > 0)
                @foreach($oldMatchesByDate as $date => $matches)
                    <div class="mb-12">
                        <h2 class="heading-font text-2xl text-gray-900 mb-6">
                            {{ \Carbon\Carbon::parse($date)->locale('ru')->isoFormat('D MMMM YYYY') }}
                        </h2>
                        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                            @foreach($matches as $match)
                                @php
                                    $matchUrl = null;
                                    if ($match->type === 'friendly' && $match->status === 'played') {
                                        $matchUrl = "/friendly-matches/{$match->id}";
                                    } elseif ($match->type === 'tournament' && $match->status === 'played') {
                                        $matchUrl = "/tournaments/{$match->stage_id}/matches/{$match->id}";
                                    }
                                @endphp
                                @if($matchUrl)
                                    <a href="{{ $matchUrl }}" class="block">
                                        @include('site.v1.modules.match-card.match-card', ['match' => $match, 'noLink' => true, 'tournamentName' => $match->tournament_name ?? null])
                                    </a>
                                @else
                                    @include('site.v1.modules.match-card.match-card', ['match' => $match, 'noLink' => true, 'tournamentName' => $match->tournament_name ?? null])
                                @endif
                            @endforeach
                        </div>
                    </div>
                @endforeach
            @else
                <div class="text-center py-12">
                    <p class="text-xl text-gray-600">Нет прошедших матчей.</p>
                </div>
            @endif
        </div>

        <!-- Recent Matches Tab -->
        <div id="tab-content-recent" class="tab-content" style="display: none;">
            @if($recentMatchesByDate->count() > 0)
                @foreach($recentMatchesByDate as $date => $matches)
                    <div class="mb-12">
                        <h2 class="heading-font text-2xl text-gray-900 mb-6">
                            {{ \Carbon\Carbon::parse($date)->locale('ru')->isoFormat('D MMMM YYYY') }}
                        </h2>
                        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                            @foreach($matches as $match)
                                @php
                                    $matchUrl = null;
                                    if ($match->type === 'friendly' && $match->status === 'played') {
                                        $matchUrl = "/friendly-matches/{$match->id}";
                                    } elseif ($match->type === 'tournament' && $match->status === 'played') {
                                        $matchUrl = "/tournaments/{$match->stage_id}/matches/{$match->id}";
                                    }
                                @endphp
                                @if($matchUrl)
                                    <a href="{{ $matchUrl }}" class="block">
                                        @include('site.v1.modules.match-card.match-card', ['match' => $match, 'noLink' => true, 'tournamentName' => $match->tournament_name ?? null])
                                    </a>
                                @else
                                    @include('site.v1.modules.match-card.match-card', ['match' => $match, 'noLink' => true, 'tournamentName' => $match->tournament_name ?? null])
                                @endif
                            @endforeach
                        </div>
                    </div>
                @endforeach
            @else
                <div class="text-center py-12">
                    <p class="text-xl text-gray-600">Нет матчей за этот период.</p>
                </div>
            @endif
        </div>

        <!-- Future Matches Tab -->
        <div id="tab-content-future" class="tab-content" style="display: none;">
            @if($futureMatchesByDate->count() > 0)
                @foreach($futureMatchesByDate as $date => $matches)
                    <div class="mb-12">
                        <h2 class="heading-font text-2xl text-gray-900 mb-6">
                            {{ \Carbon\Carbon::parse($date)->locale('ru')->isoFormat('D MMMM YYYY') }}
                        </h2>
                        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                            @foreach($matches as $match)
                                @php
                                    $matchUrl = null;
                                    if ($match->type === 'friendly' && $match->status === 'played') {
                                        $matchUrl = "/friendly-matches/{$match->id}";
                                    } elseif ($match->type === 'tournament' && $match->status === 'played') {
                                        $matchUrl = "/tournaments/{$match->stage_id}/matches/{$match->id}";
                                    }
                                @endphp
                                @if($matchUrl)
                                    <a href="{{ $matchUrl }}" class="block">
                                        @include('site.v1.modules.match-card.match-card', ['match' => $match, 'noLink' => true, 'tournamentName' => $match->tournament_name ?? null])
                                    </a>
                                @else
                                    @include('site.v1.modules.match-card.match-card', ['match' => $match, 'noLink' => true, 'tournamentName' => $match->tournament_name ?? null])
                                @endif
                            @endforeach
                        </div>
                    </div>
                @endforeach
            @else
                <div class="text-center py-12">
                    <p class="text-xl text-gray-600">Нет будущих матчей.</p>
                </div>
            @endif
        </div>
    </div>
</section>

<script>
function showTab(tabName) {
    // Скрываем все вкладки
    document.querySelectorAll('.tab-content').forEach(content => {
        content.style.display = 'none';
    });
    
    // Убираем активный класс у всех кнопок
    document.querySelectorAll('.tab-button').forEach(button => {
        button.classList.remove('active', 'border-primary', 'text-primary');
        button.classList.add('border-transparent', 'text-gray-500');
    });
    
    // Показываем выбранную вкладку
    document.getElementById('tab-content-' + tabName).style.display = 'block';
    
    // Активируем кнопку
    const button = document.getElementById('tab-' + tabName);
    button.classList.add('active', 'border-primary', 'text-primary');
    button.classList.remove('border-transparent', 'text-gray-500');
}

// Скрываем вкладки при использовании фильтров
document.getElementById('filterForm').addEventListener('submit', function() {
    const status = document.getElementById('status').value;
    const date = document.getElementById('date').value;
    const matchType = document.getElementById('match_type').value;
    if (status || date || matchType) {
        document.getElementById('tabsSection').style.display = 'none';
        // Показываем все вкладки одновременно при фильтрации
        document.querySelectorAll('.tab-content').forEach(content => {
            content.style.display = 'block';
        });
    }
});

// Показываем вкладки при сбросе фильтров
@if(!isset($hasFilters) || !$hasFilters)
showTab('old');
@endif
</script>
@endsection

